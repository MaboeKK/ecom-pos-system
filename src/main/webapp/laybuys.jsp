...
<body>
    ...
    <script >
        ...
        function fetchData(){
            fetch('UpdateServlet').then(response => {
                if(!response.ok){
                ...
                +'<div class="flex_item">R '+item.total+'.00</div>'+
                '<div class="flex_item">'+item.status+'</div>'+
                '<div class="flex_item link">'+
                '<a class="purchase" href=CustomerByOrderServlet?order='+item.id+'>Payment</a></div></div>';
